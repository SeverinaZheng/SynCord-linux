...
static inline void rwmutex_lock(struct rwmutex *lock, int is_reader)
{
	...
	__aqm_lock_slowpath(lock, TASK_UNINTERRUPTIBLE, is_reader, false, 0);
}

static inline void bpf_rwmutex_lock(struct rwmutex *lock, int is_reader, int policy_id)
{
        if (likely(cmpxchg(&lock->locked_no_stealing, 0, 1) == 0))
                return;

        __aqm_lock_slowpath(lock, TASK_UNINTERRUPTIBLE, is_reader, true, policy_id);
}
...
static inline void bpf_rwmutex_unlock(struct rwmutex *lock, int policy_id)
{
        xchg(&lock->locked, 0);
}
...
static inline void shuffle_waiters(struct rwmutex *lock, struct rwaqm_node *node,
				   int is_next_waiter, bool is_bpf, int policy_id)
{
...
static int __aqm_lock_slowpath(struct rwmutex *lock, long state, int is_reader, bool is_bpf, int policy_id)
{
	...
	if (wcount)
		...
		if (sleader) {
			...
			shuffle_waiters(lock, node, true, is_bpf, policy_id);
			/* disable_stealing(lock); */
		}
        }
...
static inline void bpf_aqm_write_lock_slowpath(struct rw_semaphore *sem, int policy_id)
{
        bpf_rwmutex_lock(&sem->wait_lock, false,policy_id);
        ...
        do {
                ...
                atomic_long_cond_read_acquire(&sem->cnts, VAL == 0);
                                             ...
                                             RWAQM_W_LOCKED) != 0);
     unlock:
        bpf_rwmutex_unlock(&sem->wait_lock,policy_id);
}
...
void bpf_down_write(struct rw_semaphore *sem, int policy_id)
{
        might_sleep();
        ...
        bpf_aqm_write_lock_slowpath(sem, policy_id);
}
